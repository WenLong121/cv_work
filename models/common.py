...
class Conv(nn.Module):
    ...
    def forward(self, x):
        return self.act(self.bn(self.conv(x.half())))

    def fuseforward(self, x):
        return self.act(self.conv(x.half()))
...
class SE_Fusion(nn.Module):
    ...
    def forward(self, x):
        rgb_fea = x[0].float() # 获取rgb
        ir_fea = x[1].float()  # 获取红外
        bs, c, h, w = rgb_fea.shape 
        ##########
        in_channels_rgb = rgb_fea.shape[1]
        model_se = SEAttention(in_channels_rgb).cuda() # 加载SE注意力
        rgb_fea_se = model_se(rgb_fea) 
        ir_fea_se = model_se(ir_fea)

        sigmoid = nn.Sigmoid()
        gap = nn.AdaptiveAvgPool2d(1)
        batch_size, channels, _, _ = rgb_fea.size()
        # 红外和rgb 全局池化和激活
        sub_vi_ir = rgb_fea - ir_fea 
        rgb_fea = sub_vi_ir * sigmoid(gap(sub_vi_ir))
        sub_ir_vi = ir_fea - rgb_fea
        ir_fea = sub_ir_vi * sigmoid(gap(sub_ir_vi))
        #########
        if self.training == True:
            rgb_fea = F.interpolate(rgb_fea, size=([h, w]), mode='nearest')
        else:
            rgb_fea = F.interpolate(rgb_fea, size=([h, w]), mode='bilinear')
        new_rgb_fea = rgb_fea + rgb_fea_se
        if self.training == True:
            ir_fea = F.interpolate(ir_fea, size=([h, w]), mode='nearest')
        else:
            ir_fea = F.interpolate(ir_fea, size=([h, w]), mode='bilinear')
        new_ir_fea = ir_fea + ir_fea_se
        # fusion
        new_fea = self.concat([new_rgb_fea, new_ir_fea])  
        y = self.conv1x1_out(new_fea)
        return y
